...
def process_file(events_path, seq_lookup):
    ...
    # Reconstruct full session stream
    full_stream_ab = []
    item_rows = []
    ...
    for idx, row in df.iterrows():
        ...
        # We need "Tone Identity" (0 for 350Hz, 1 for 500Hz)
        # Let's standardize: 0=Low, 1=High. 
        # If low-high: A->0, B->1.
        # If high-low: A->1, B->0.
        ...
        for pos, char in enumerate(ab_seq):
            ...
            # Repetition (Local)
            # Calculated per sequence or globally? Legacy script did per sequence usually for simple GLMs, 
            # but here "Repetition" is often "Current == Previous".
            # Let's do it based on the `ab_seq` string for robustness (within-sequence repetition).
            if pos == 0:
                rep = 0 # No previous in sequence
            else:
                rep = 1 if ab_seq[pos] == ab_seq[pos-1] else 0
                
            full_stream_ab.append(char)
            
            item_rows.append({
                'session_id': idx, # Using trial index as session proxy for now or extract from filename
                'trial_index': idx,
                'pos': p_idx,
                'tone_char': char,
                'tone_id': tone_id,
    ...
    # Batch calculate surprisal for the whole session stream?
    # Or reset per sequence? 
    # The prompt says "Session Structure: 15 Habituation ...". Usually Ideal Observer runs continuously.
    # Legacy script manual said "RESET_POLICY = session".
    # So we run on full_stream_ab.
    
    surprisals = calculate_surprisal(full_stream_ab, tau=TAU)
    
    # Merge back
    out_df = pd.DataFrame(item_rows)
    out_df['surprisal'] = surprisals
    
    # Add Interaction Term? No, let the GLM script do that. We provide raw predictors.
